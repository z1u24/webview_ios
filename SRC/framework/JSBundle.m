...
- (void)callJS:(NSNumber *)listenerID code:(CallJSType)code params:(NSArray *)params {
    ...
    //NSString *fullCode = "window['onWebViewPostMessage']('%@', '%@')"
...
- (void)sendMessage:(NSArray *)params {
    ...
    if (nativeID.intValue == 0) {
        if ([funcName isEqualToString:@"init"]) {
            @synchronized (self) {
                [self initImpl:clazz params:params];
                //obj setJSBundle(this);
            }
        } else {
            // 类方法
            ...
            if ([clazz respondsToSelector:sel]) {
                ...
                [self callJSError:NSStringFromClass(clazz) funcName:funcName msg:@"class methed funcName not found"];
            }
        }
    } else {
        // close方法
        ...
        if ([object respondsToSelector:sel]) {
            //在执行方法的时候添加JSBundle对象
            NSMutableArray *mutableParams = [[NSMutableArray alloc] initWithArray:params];
            [mutableParams addObject:self];
            [object performSelector:sel withObject:mutableParams];
        } else {
            [self callJSError:NSStringFromClass(clazz) funcName:funcName msg:@"object method funcName not found"];
        }
    }
}
...
- (void)initImpl: (Class)clazz params: (NSArray*)array {
    ...
    [self callJS:listenerID code:0 params:[NSArray arrayWithObjects:[NSNumber numberWithInt:identity],nil]];
}
