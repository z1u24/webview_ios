...
var Ethereumjs = function Ethereumjs() {
...
var EthWallet = function () {
    ...
    _createClass(EthWallet, [{
        ...
        value: function exportKeystore(passwd) {
            ...
            var wlt = Ethereumjs().Wallet.fromPrivateKey(decrypted);
        ...
        value: function selectAddress(index) {
            ...
            var rootNode = Ethereumjs().WalletHD.fromMasterSeed(Ethereumjs().Buffer.Buffer(masterSeed, 'hex'));
        ...
        value: function fromMnemonic(mnemonic, language) {
            ...
            var rootNode = Ethereumjs().WalletHD.fromMasterSeed(Ethereumjs().Buffer.Buffer(seedBuffer, 'hex'));
        ...
        value: function fromSeed(seed, language) {
            ...
            var rootNode = Ethereumjs().WalletHD.fromMasterSeed(Ethereumjs().Buffer.Buffer(seed, 'hex'));
            var hdwlt = rootNode.derivePath(DEFAULT_DERIVE_PATH);
            var wlt = hdwlt.getWallet();
            var gwlt = new EthWallet();
            gwlt._address = wlt.getChecksumAddressString();
        ...
        value: function fromKeyStore(v3string, passwd) {
            var wlt = Ethereumjs().Wallet.fromV3(v3string, passwd, true);
            var gwlt = new EthWallet();
            gwlt._address = wlt.getChecksumAddressString();
            gwlt._privKey = wlt.getPrivateKey().toString('hex');
            return gwlt;
        ...
        value: function fromPrivateKey(privKey) {
            var sk = Ethereumjs().Buffer.Buffer(privKey, 'hex');
            var wlt = Ethereumjs().Wallet.fromPrivateKey(sk);
            var gwlt = new EthWallet();
            gwlt._address = wlt.getChecksumAddressString();
...
exports.initWeb3 = function () {
    if (!exports.web3) {
        exports.web3 = new web3_min_1.Web3(new web3_min_1.Web3.providers.HttpProvider(config_2.getEthApiBaseUrl()));
    }
};
})