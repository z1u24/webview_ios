 ...
 */
// ================================ 导入
var memstore_1 = require("../../../earn/client/app/store/memstore");
...
var App = function (_widget_1$Widget) {
    ...
    _createClass(App, [{
        ...
        value: function setList() {
            var resList = [];
            for (var item in this.props.allTabBar) {
                if (modulConfig_1.getModulConfig(this.props.allTabBar[item].modulName)) {
                ...
                return regeneratorRuntime.wrap(function _callee$(_context) {
                    while (1) {
                        switch (_context.prev = _context.next) {
                            case 0:
                                identfy = this.props.tabBarList[index].modulName;

                                if (!(this.props.isActive === identfy)) {
                                    _context.next = 3;
                                    break;
                                }
...
// ===================================================== 本地
// ===================================================== 立即执行
memstore_2.register('flags/level_2_page_loaded', function (loaded) {
    ...
    if (localStorage.getItem('kickOffline')) {
        ...
        tools_1.kickOffline(); // 踢人下线提示
    }
});
// 用户信息变化
memstore_2.register('user/info', function (userInfo) {
    if (userInfo) {
        pull_1.setUserInfo();
    }
});
// 登录状态成功
memstore_2.register('user/isLogin', function (isLogin) {
    if (isLogin) {
        // 余额
        ...
        pull_1.getRealUser();
        // 用户基础信息
        pull_1.getUserInfoFromServer(memstore_2.getStore('user/conUid'));
    }
});
// 获取随机数成功
memstore_2.register('user/conRandom', function () {
    // eth gasPrice
...
// 语言配置
memstore_2.register('setting/language', function (r) {
    lang_1.setLang(r);
});
// 监听活动页面
