...
var TransactionDetails = function (_widget_1$Widget) {
    ...
    _createClass(TransactionDetails, [{
        ...
        value: function init() {
            ...
            this.props = Object.assign({}, this.props, { tx: tx, hashShow: tools_1.parseAccount(tx.hash), timeShow: tools_1.timestampFormat(tx.time), statusShow: obj.text, statusIcon: obj.icon, minerFeeUnit: tx.currencyName !== 'BTC' ? 'ETH' : 'BTC', canResend: tools_1.canResend(tx), qrcode: "" + qrcodePrefix + tx.hash, webText: webText });
        }
    }, {
        key: "backPrePage",
        ...
        value: function resendClick() {
            ...
            if (tx.txType === interface_1.TxType.Recharge) {
                root_1.popNew('app-view-wallet-cloudWallet-recharge', { tx: tx, currencyName: this.props.tx.currencyName });
            } else {
                root_1.popNew('app-view-wallet-transaction-transfer', { tx: tx, currencyName: this.props.tx.currencyName });
            }
            this.ok && this.ok();
        }
    }, {
        ...
        value: function copyToAddr() {
            tools_1.copyToClipboard(this.props.tx.toAddr);
            tools_1.popNewMessage(this.language.tips[2]);
        }
    }, {
        key: "copyFromAddr",
        value: function copyFromAddr() {
            tools_1.copyToClipboard(this.props.tx.fromAddr);
            tools_1.popNewMessage(this.language.tips[2]);
        }
    }, {
        key: "copyHash",
        value: function copyHash() {
            tools_1.copyToClipboard(this.props.tx.hash);
            tools_1.popNewMessage(this.language.tips[2]);
        }
    }, {
        key: "openNewWeb",
        value: function openNewWeb() {
            ...
            root_1.popNew('app-components-openLink-openLink', {}, function () {
                var title = _this2.props.tx.currencyName === 'BTC' ? 'Blockchain' : 'Etherscan';
                native_1.openNewActivity(_this2.props.qrcode, title);
            });
        }
    }, {
