...
var CoinConvert = function (_widget_1$Widget) {
    ...
    _createClass(CoinConvert, [{
        ...
        value: function rateDetail() {
            ...
            var tips = this.language.tips[5] + this.props.inMinerFee + ' ' + this.props.inCurrency;
            // tslint:disable-next-line:max-line-length
        ...
        value: function goHistory() {
            root_1.popNew('app-view-wallet-coinConvert-convertHistory', { currencyName: this.props.outCurrency });
        }
    }, {
        key: "setProps",
        value: function setProps(props, oldProps) {
            ...
            // ZRX   BAT
            this.props = Object.assign({}, this.props, { outCurrency: outCurrency,
                inCurrency: inCurrency, pair: '', maxLimit: 0, minimum: 0, rate: 0, outMinerFee: 0, inMinerFee: 0, timer: 0, outBalance: 0, outAmount: 0, receiveAmount: 0, curOutAddr: '', curInAddr: '' });
            this.init();
            this.updateMinerFee();
        }
        // 更新矿工费
        ...
        value: function init() {
            ...
            this.setPair();
            // 获取出币币种的余额和当前使用地址
            this.props.curOutAddr = tools_1.getCurrentAddrByCurrencyName(this.props.outCurrency);
            this.props.outBalance = tools_1.getCurrentAddrInfo(this.props.outCurrency).balance;
            // 获取入币币种的当前使用地址
            this.props.curInAddr = tools_1.getCurrentAddrByCurrencyName(this.props.inCurrency);
        ...
        value: function marketInfoUpdated() {
            ...
            clearTimeout(this.props.timer);
            pullWallet_1.getMarketInfo(this.props.pair);
        ...
        value: function inCurrencySelectClick() {
            ...
            var dataList = [];
            data.forEach(function (element) {
                if (element.symbol !== _this3.props.outCurrency) {
                    // 去掉出币币种
                    dataList.push(element.symbol);
                }
            });
            root_1.popNew('app-components-chooseCurrency-chooseCurrency', { list: dataList, selected: dataList.indexOf(this.props.inCurrency) }, function (r) {
                var currencyName = dataList[r];
                if (_this3.props.inCurrency === currencyName) return;
                if (_this3.props.outCurrency === currencyName) {
                    var index = dataList.indexOf(currencyName);
                    _this3.props.outCurrency = dataList[(index + 1) % dataList.length];
                }
                _this3.props.inCurrency = dataList[r];
                _this3.props.outAmount = 0;
                _this3.props.receiveAmount = 0;
                _this3.init();
                _this3.paint();
            });
        }
                    ...
                    while (1) {
                        switch (_context3.prev = _context3.next) {
                            case 0:
                                ...
                                outCurrency = this.props.outCurrency;
                            ...
                            case 11:
                                // tslint:disable-next-line:max-line-length
                                content = [this.language.tips[6] + outAmount + outCurrency, this.language.tips[7] + this.props.receiveAmount + this.props.inCurrency];
                                _context3.next = 14;
                            ...
                            case 17:
                                ...
                                withdrawalAddress = this.props.curInAddr; // 入账币种的地址

                                returnAddress = this.props.curOutAddr; // 失败后的退款地址

                                pair = this.props.pair; // 交易对
        ...
        value: function setTemRecord(hash, fromAddr, pay, outCurrency, inCurrency, rate) {
            ...
            console.log('tx', tx);
            var addrLowerCase = this.props.curOutAddr.toLowerCase();
