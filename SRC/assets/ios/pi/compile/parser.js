    ...
    _createClass(Parser, [{
        ...
        value: function setRule(s, cfgs) {
            ...
            for (var _iterator = cfgs, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
                ...
                buildCfg(_r, this.tokenMap, this.syntaxMap);
            ...
            do {
                if (this.lastIndex >= this.last.length) {
                    ...
                    if (this.scanner.scan(t)) {
                        ...
                        if (!t.rule) t.rule = this.defaultRule;
                        t.loc = this.last.length;
                        this.lastIndex++;
                        this.last.push(t);
                        this.cur = t;
                        if (t.index > this.lastMatch.index) this.lastMatch = t;
                    } else {
                        this.lastIndex++;
                        this.cur = null;
                    }
                } else this.cur = this.last[this.lastIndex++];
        ...
        value: function setCur(lastIndex, lastDeep) {
            if (lastIndex === this.lastIndex) return;
            var reback = false;
        ...
        value: function parseRule(rules) {
            ...
            for (var _iterator3 = arr, _isArray3 = Array.isArray(_iterator3), _i3 = 0, _iterator3 = _isArray3 ? _iterator3 : _iterator3[Symbol.iterator]();;) {
                ...
                if (!rr) throw new Error("parser, parseRule fail, invalid rule: " + rule);
                var s = new Syntax();
                s.type = rr.rule.name;
                s.right = [];
                s.token = this.cur;
        ...
        value: function belongNote() {
            for (var _iterator4 = this.last, _isArray4 = Array.isArray(_iterator4), _i4 = 0, _iterator4 = _isArray4 ? _iterator4 : _iterator4[Symbol.iterator]();;) {
                ...
                if (r.rule.note) addNote(this.last, r);
            }
        }
    }]);
...
var merge = function merge(map) {
    ...
    if (len < size) throw new Error("parser, rule merge fail, name: " + name);
};
// 构建词法规则
var buildCfg = function buildCfg(cfg, map, syntaxMap) {
    ...
    if (cfg.lbp) r.lbp = cfg.lbp;
    if (cfg.rbp) r.rbp = cfg.rbp;
    if (cfg.suf) r.suf = cfg.suf;
    if (cfg.ignore) r.ignore = cfg.ignore;
    if (cfg.note) r.note = cfg.note;
    if (cfg.nud) {
...
var makeRuleArray = function makeRuleArray(s, syntaxMap, func) {
    ...
    for (var _iterator8 = ss, _isArray8 = Array.isArray(_iterator8), _i8 = 0, _iterator8 = _isArray8 ? _iterator8 : _iterator8[Symbol.iterator]();;) {
        ...
        _s3 = _s3.trim();
        var rr = syntaxMap.get(_s3);
        if (!rr) throw new Error("parser, makeRuleArray fail, invalid name: " + _s3);
        arr.push(func(rr));
    }
    return arr;
};
...
var buildNext = function buildNext(r) {
    r.lbp = r.lbp || 0;
    r.rbp = r.rbp || r.lbp || 0;
    if (r.nud === itself && r.rbp !== 0) r.nud = r.rbp > 0 ? prefix : null;
    if (r.led === error && r.lbp) r.led = r.suf ? suffix : infix;
};
/**
...
var makeRuleEntry = function makeRuleEntry(r, re, map) {
    ...
    if (!func) throw new Error("parser, make rule fail, invalid name: " + re.type);
...
var addNote = function addNote(last, note) {
    ...
    if (note.rule.note < 0) {
        ...
        for (var i = note.loc - 1; i >= 0; i--) {
            r = last[i];
            if (r.rule.note || r.rule.ignore) continue;
            s = r.parent;
            if (!s.right) s = s.parent;
...
var ruleTab = {
    ...
    "terminal": function terminal(r, re, map) {
        ...
        return function (p, s) {
            var sss = re.str;
            if (!p.cur || p.cur.type !== str) return false;
    ...
    "builtIn": function builtIn(r, re, map) {
        ...
        if (!func) throw new Error("scanner, make rule fail, invalid builtIn: " + re.value);
        return function (p, s) {
            var ss = func(p);
            if (!ss) return false;
            if (re.option !== "") {
