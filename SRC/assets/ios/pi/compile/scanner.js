...
var stateScan = function stateScan(s, ss, t) {
    ...
    if (!type) {
        ...
        for (; _i3 < _len; _i3++) {
            ...
            if (s.reset) mr.ok.push(rule);else s.reset = false;
        }
...
var build = function build(rule, arr, map) {
    ...
    if (rule.entry.type === "terminal" && rule.name === rule.entry.value || rule.entry.type === "and" && rule.name === getAndTerminalValue(rule.entry.childs)) r.nameType = true;
    arr.push(r);
    map.set(rule.name, r);
};
// 创建词法规则函数
var makeRuleEntry = function makeRuleEntry(re, map) {
    ...
    if (!func) throw new Error("scanner, make rule fail, invalid name: " + re.type);
    return func(re, map);
};
// 构建与规则中终结符的值
var getAndTerminalValue = function getAndTerminalValue(arr) {
    for (var _iterator3 = arr, _isArray3 = Array.isArray(_iterator3), _i4 = 0, _iterator3 = _isArray3 ? _iterator3 : _iterator3[Symbol.iterator]();;) {
        ...
        if (e.type === "terminal") return e.value;
    }
};
// 创建词法规则函数数组
...
var merge = function merge(arr, map) {
    ...
    if (len < arr.length) throw new Error("scanner, rule merge fail, name: " + name);
};
// 词法规则函数表
var ruleTab = {
    ...
    "and": function and(re, map) {
        ...
        return function (s) {
            ...
            for (var j = 1, len = arr.length; j < len; j++) {
                ...
                var _r = arr[j](s);
                if (!_r) return false;
                if (s.lastIndex === i) continue;
                if (old >= 0) {
                    if (old !== s.lastIndex) {
                        s.setCur(i);
                        return false;
                    }
                } else old = s.lastIndex;
            }
            return true;
        };
    },
    ...
    "repeat": function repeat(re, map) {
        ...
        return function (s) {
            ...
            if (!r) return false;
            while (r) {
                if (s.lastIndex === i) throw new Error("scanner, repeat fail, endless loop: " + re.value + ", line: " + s.line + ", column: " + s.column);
                r = func(s);
            }
            return true;
        };
